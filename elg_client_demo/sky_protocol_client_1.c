...
int sky_encode_req_bin_1(unsigned char *buff, int buff_len,
    ...
    if (acnt > 0)
        crypt_len += acnt * (int) sizeof(struct ap_t) + 2;
    if (bcnt > 0)
        crypt_len += bcnt * (int) sizeof(struct ble_t) + 2;
    if (gcnt > 0)
        crypt_len += gcnt * (int) sizeof(struct gps_t) + 2;
    ...
    if (ccnt > 0) {
        ...
        case DATA_TYPE_GSM:
            sz = (int) sizeof(struct gsm_t);
            break;
        case DATA_TYPE_CDMA:
            sz = (int) sizeof(struct cdma_t);
            break;
        case DATA_TYPE_UMTS:
            sz = (int) sizeof(struct umts_t);
            break;
        case DATA_TYPE_LTE:
            sz = (int) sizeof(struct lte_t);
            break;
        default:
            return -1;
        }
    ...
    if (pad_len < 2)
        pad_len += 16; // we need to add the 2 verification fields
...
int sky_decode_resp_bin_1(unsigned char *buff, int buff_len, int data_len,
    ...
    // get basic location from buffer
    memcpy(&cresp->location, p, LOCATION_T_SIZE);
    p += LOCATION_T_SIZE;

    // get device MAC address
    if (cresp->payload_type == LOCATION_RQ_MAC
            || cresp->payload_type == LOCATION_RQ_ADDR_MAC) {
        memcpy(&cresp->MAC, p, 6);
        p += 6;
    }

    if (cresp->payload_type == LOCATION_RQ
            || cresp->payload_type == LOCATION_RQ_MAC) {
        return 0;
    }
